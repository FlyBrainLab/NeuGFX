...
generatePinMap = function (IOData, IOSynapses, IOName) {
    ...
    var centerPixel = 700;
    var allRadius = 700;
    var nNeurons = IOData.inputs.length;

    neuronInY = function (i) { return centerPixel + allRadius * Math.cos((i) / nNeurons * 2 * Math.PI )};
    neuronInX = function (i) { return centerPixel + allRadius * Math.sin((i) / nNeurons * 2 * Math.PI  )};

    neuronInYr = function (i,r) { return centerPixel + r * Math.cos((i) / nNeurons * 2 * Math.PI  )};
    neuronInXr = function (i,r) { return centerPixel + r * Math.sin((i) / nNeurons * 2 * Math.PI  )};
    ...
    for (var i = 0; i < IOData.inputs.length; i++) {
        var neuron = svgObj.append("circle")
            .attr("cx", neuronInX(i) )
            .attr("cy", neuronInY(i) )
        ...
        var neuron_line = svgObj.append("line")
            .attr("x1", neuronInXr(i, allRadius-3*neuronHeight/2))
            .attr("y1", neuronInYr(i, allRadius-3*neuronHeight/2))
            .attr("x2", neuronInXr(i, allRadius-neuronHeight/2))
            .attr("y2", neuronInYr(i, allRadius-neuronHeight/2))
            .attr("label", IOData.inputs[i])
            .attr("class", "neuron_class")
            .attr("selected", "true")
            .attr("tooltip-data", IOData.inputs[i] + " :: " + "This is an OTP Node.")
        ...
        var neuron_line2 = svgObj.append("line")
            .attr("x1", neuronInXr(i, allRadius+neuronHeight/2))
            .attr("y1", neuronInYr(i, allRadius+neuronHeight/2))
            .attr("x2", neuronInXr(i, allRadius+3*neuronHeight/2))
            .attr("y2", neuronInYr(i, allRadius+3*neuronHeight/2))
            .attr("label", IOData.inputs[i])
            .attr("class", "neuron_class")
            .attr("selected", "true")
        ...
        var neuron_circle = svgObj.append("circle")
            .attr("cx", neuronInXr(i, allRadius+2*neuronHeight))
            .attr("cy", neuronInYr(i, allRadius+2*neuronHeight))
            .attr("r", neuronHeight/2+1)
        ...
        var neuronText2 = svgObj.append("text")
            .attr("x", neuronInXr(i, allRadius+2*neuronHeight))
            .attr("y", neuronInYr(i, allRadius+2*neuronHeight))
            .attr("dy", ".32em")
            .text('BSG')
            .style("text-anchor", "middle");
        var neuronText = svgObj.append("text")
            .attr("x", neuronInXr(i, allRadius+0*neuronHeight))
            .attr("y", neuronInYr(i, allRadius+0*neuronHeight))
            .attr("dy", ".32em")
            .text(IOData.inputs[i])
            .style("text-anchor", "middle");
    }

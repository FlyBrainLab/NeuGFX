window.neuron_selector = ".neuron_class";
...
// window.IOName = window.IOName;
window.fbl.experimentConfig[window.IOName].updated = [];

window.updateCircuit = function () {
    if (IOName in window.fbl.experimentConfig) {
        if ('disabled' in window.fbl.experimentConfig[IOName]) {
            let disabled_temp = Array.from(window.fbl.experimentConfig[window.IOName].disabled);
            window.fbl.experimentConfig[window.IOName].disabled = [];
            var arrayLength = disabled_temp.length;
            for (var i = 0; i < arrayLength; i++) {
                if (disabled_temp[i].includes('->')) {
                    window.fbl.experimentConfig[window.IOName].disabled.push(disabled_temp[i]);
                }
            }
        }
        else {
    ...
    svgObj.selectAll(neuron_selector).each(function (d, i) {
        if (this.getAttribute("selected") == "false") {
            window.fbl.experimentConfig[window.IOName].disabled.push(d3.select(this).attr("label"));
        }
    });
...
svgObj.selectAll("*").on("mouseover", function (d, i) {
    if (this.getAttribute("label") != null) {
        ...
        if (this.getAttribute("label") in window.fbl.experimentConfig[window.IOName]) {
            document.getElementsByClassName("fbl-info-desc")[0].innerHTML = JSON.stringify(window.fbl.experimentConfig[window.IOName][this.getAttribute("label")]);
        }
...
svgObj.selectAll(neuron_selector).on("click", function (d, i) {
    //svgObj.selectAll(neuron_selector).each(function (d, i) {
    //d3.select(this).style("opacity", "0.3");
    var _this = this;
    var $this = $(this);
...
window._neuGFX.mods.FlyBrainLab.addFBLPath("Central Complex", function () { window.fbl.loadFBLSVG(window.IOName, function () { window.fbl.loadSubmodule('data/FBLSubmodules/onCXLoad.js'); console.log("Submodule loaded.") }); });
